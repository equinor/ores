...
# -*- coding: utf-8 -*-
"""
rev_propertytypes.py — ADME/OSDU-compliant manifest generator
Creates reference-data records for:
  kind = osdu:wks:reference-data--ReservoirEstimatedVolumePropertyType:1.0.0

No facet logic here. Facets (if any) are managed separately.

Spec:
- ReservoirEstimatedVolumePropertyType: accepted list used by ReservoirEstimatedVolume.
"""
...
def main():
    ap = argparse.ArgumentParser(description="Generate ReservoirEstimatedVolumePropertyType manifest (no facets).")
    ...
    ap.add_argument("--out", default="reftypes_propertytypes.json")
    args = ap.parse_args()
