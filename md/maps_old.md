...
---
## 9b. When FIRP Adds Value vs When It Is Overhead

A common question from teams ingesting OpenWorks data:

> "It looks like everything important belongs to the representation object, and then interpretation and boundary are just extra redundant objects imposed by the theoretical FIRP hierarchy that don't carry useful information. We see empty interpretation/feature objects (e.g. from LMK) that add no information beyond the representation. Should we always create them?"
...
If none of those conditions are true - and won't become true - skip the objects.

### Summary

- **Don't create redundant FIRP objects just to satisfy a theoretical schema.**
- **Do create them when the source data genuinely has a shared geologic identity** that multiple representations reference.
- For OW attribute maps: `GenericBinGrid` + `GenericProperty` is the right target. No fabricated hierarchy.
- For genuine structural horizons with a named geologic boundary: create the full chain - it enables strat linkage, graph search, and impact analysis.
- The FIRP pattern is a *domain ontology*, not a bureaucratic filing requirement. If the source data doesn't carry interpretation-level semantics, the objects shouldn't exist in the target either.
