...
def build_statistics_manifest(raw_manifest: Dict[str, Any], facet_roles: Dict[str, Any], id_prefix: str, use_uuid: bool) -> Dict[str, Any]:
    ...
    for (seg, zone), grp in groups.items():
        ...
        agg_rows.append(rec)
    # Add TOTAL rows
    for seg in set(k[0] for k in groups):
        rec = {"SegmentID": seg, "Zone": "TOTAL"}
        rec.update(_compute_stats([r for k, v in groups.items() if k[0] == seg for r in v], properties))
        agg_rows.append(rec)
    rec = {"SegmentID": "TOTAL", "Zone": "TOTAL"}
